...
def regtea14():
    if request.method == 'GET':
        ...
        return render_template('14-regtea.html', courses=courses)
    else:
        id = request.form['cid']
...
@app.route('/15-otm')
def otm():
    # 1.通过 course 对象查询对应的 teachers 们

    # 查询 id为3的课程的信息
    # course = Course.query.filter_by(id=3).first()
    # course.teachers:得到课程对应的老师们的查询对象 -> AppenderBaseQuery(BaseQuery)
    # course.teacher.all() 得到课程对应的老师的信息们 -> list
    # teas = course.teachers.all()
    # print(teas)
    # print("type:" , type(teas))
    # print("课程名称:%s" % course.cname)
    # print("授课讲师:")
    # for t in teas:
    #     print("姓名:%s,年龄:%s" % (t.tname,t.tage))
...
@app.route('/16-otm-exer')
def otm_exer():
    # 接收前端传递过来的cid(可能有cid也可能没有cid,如果没有的话则默认为0即可)
    ...
    if cid == 0:
        ...
        # 方案1
        teachers = Teacher.query.filter_by(cid=cid).all()

        # 方案2
        # course = Course.query.filter_by(id=cid).first()
        # teachers = course.teachers.all()

    return render_template('16-otm-exer.html', courses=courses, teachers=teachers, cid=cid)
...
@app.route('/18-mtm-exer',methods=['GET','POST'])
def mtm_exer():
    if request.method == 'GET':
        courses = Course.query.all()
        return render_template('18-mtm-exer.html',courses=courses)
    else:
        # 接收前端传递过来的数据
        ...
        db.session.add(stu)
        # 查询cids们对应的对象们
        courses = db.session.query(Course).filter(Course.id.in_(cids)).all()
        for cou in courses:
